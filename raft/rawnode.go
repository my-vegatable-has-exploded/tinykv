...
func isSoftStateEqual(a *SoftState, b *SoftState) bool {
...
func (rn *RawNode) HasReady() bool {
	...
	if len(r.RaftLog.nextEnts()) != 0 || len(r.RaftLog.unstableEntries()) != 0 {
		return true
	}
	if !IsEmptySnap(r.RaftLog.pendingSnapshot) {
		return true
...
// last Ready results.
func (rn *RawNode) Advance(rd Ready) { // Todo@wy this kind of rewrite may not be reasonalbe , i don't know if node handle ready serially
	// Your Code Here (2A).
	if rd.SoftState != nil {
...
func (rd *Ready) appiledCursor() uint64 {
	if n := len(rd.CommittedEntries); n > 0 {
		return rd.CommittedEntries[n-1].Index
	}
	if snap := rd.Snapshot; !IsEmptySnap(&snap) {
